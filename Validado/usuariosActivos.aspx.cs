...
{
    public partial class usuariosActivos : System.Web.UI.Page
    {
        ...
        protected void BindGridView()
        {
            try
            {
                ...
                string cmd2 = "SELECT  "+
                              ...
                              " WHERE (PlanEmpresa.PlanActivo = 1) " + ViewState["sWhere"] + " ORDER BY Cliente.ClienteNombre, Usuario.UsuarioCedula ASC ";
                ...
                try {
                    ...
                    string Inactivos = "SELECT Usuario.UsuarioID as UsuarioId, " +
                              ...
                              " Usuario.UsuarioID INNER JOIN UsuarioRol ON Usuario.UsuarioID = " +
                              " UsuarioRol.UsuarioID INNER JOIN Cliente ON UsuarioRol.ClienteID = Cliente.ClienteID " +
                              " WHERE (PlanEmpresa.PlanActivo = 0) " + ViewState["sWhere"] + " ORDER BY PlanEmpresa.fechaUltimo DESC";

                    string InactivosCant = "SELECT COUNT(Usuario.UsuarioID) as UsuarioId " +
                              " FROM PlanEmpresa INNER JOIN Usuario ON PlanEmpresa.UsuarioID = " +
                              " Usuario.UsuarioID INNER JOIN UsuarioRol ON Usuario.UsuarioID = " +
                              " UsuarioRol.UsuarioID INNER JOIN Cliente ON UsuarioRol.ClienteID = Cliente.ClienteID " +
                              " WHERE (PlanEmpresa.PlanActivo = 0) " + ViewState["sWhere"];
        ...
        protected void ImgbtnArchivo_Click(object sender, ImageClickEventArgs e)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < usuarioIn.Length; i++)
                {
                    string query = "SELECT TOP 2 PlanEmpresaFecha FROM PlanEmpresaHistorial WHERE PlanEmpresaUsuarioID = " + usuarioIn[i];
