...
{
    public partial class AsignarTema : System.Web.UI.Page
    {
        ...
        static string ClaseID="";

        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                string sSelectSQL = "SELECT DISTINCT(c.ClaseID) AS VAL, c.ClaseDescripcion AS TXT FROM Clase c, ClasePlantilla cp WHERE ClaseActiva = 1 " +
                                    " AND c.ClaseID = cp.ClaseID ORDER BY VAL";
                Utilidades.CargarListado(ref dplClaseProfesor, sSelectSQL, cn, ref Err, true);
                Utilidades.CargarListado(ref dplClaseAdd, sSelectSQL, cn, ref Err, true);
                if (dplClaseProfesor.Items.Count == 1)
                {
                    dplClaseProfesor.Items.Clear();
                    dplClaseProfesor.Items.Add(new ListItem("No Tiene Clases Asignadas", ""));
                    dplClaseAdd.Items.Clear();
                    dplClaseAdd.Items.Add(new ListItem("No Tiene Clases Asignadas", ""));
                    Agregar.Enabled = false;
                }
                string sSelectSQL2 = "SELECT NivelID AS VAL, NivelNombre AS TXT FROM Nivel WHERE NivelActivo = 1 ORDER BY VAL";
                Utilidades.CargarListado(ref dplNivelAdd, sSelectSQL2, cn, ref Err, true);
            }
        }

        protected void BindGridView(string ClaseID)
        {
            try
            {
                cn.Open();
                ...
                string cmd2 = "SELECT Elemento.ElementoID as ElementoID, Elemento.ElementoNombre as ElementoNombre," +
                             ...
                             " Nivel ON Clase_Nivel_Elemento.NivelID = Nivel.NivelID WHERE Clase.ClaseID = "+ClaseID;
        ...
        protected void dplClaseProfesor_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (dplClaseProfesor.SelectedValue != "")
            {
                ClaseID = dplClaseProfesor.SelectedValue;
                BindGridView(ClaseID);
            }
        }
