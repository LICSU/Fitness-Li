...
namespace LicsuWeb.Validado
{
    public partial class SubirVideo : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack) 
            {
                sSelectSQL = "SELECT c.ClaseID as VAL, c.ClaseDescripcion as TXT FROM Clase c, ClasePlantilla cp WHERE cp.ProfesorID = "+_autenticado.UsuarioID;
            ...
            if (dplClases.SelectedValue != "")
            {
                if (flpVideos.HasFile)
                {
                    ...
                    if (fileExt == ".txt")
                    {
                        try
                        {
                            string savePath = Server.MapPath("~/Validado/Videos/");
